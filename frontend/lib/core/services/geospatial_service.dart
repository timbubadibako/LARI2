...
class GeospatialService {
  ...
  GeospatialService() {
    try {
      ...
      debugPrint('H3_LOAD_FAILED: Geospatial hex features will be disabled. $e');
      _h3Available = false;
    }
  }

  bool get isAvailable => _h3Available;
  ...
  String coordinateToH3Index(double latitude, double longitude, {int resolution = 9}) {
    if (!_h3Available || _h3 == null) return '0';
  ...
  List<GeoCoord> getH3Boundary(String h3IndexString) {
    if (!_h3Available || _h3 == null) return [];
    try {
  ...
  GeoCoord getH3Center(String h3IndexString) {
    if (!_h3Available || _h3 == null) return GeoCoord(lat: 0, lon: 0);
    try {
      final h3Index = BigInt.parse(h3IndexString, radix: 16);
